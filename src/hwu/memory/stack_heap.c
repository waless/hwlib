...
static void* allocate_from_top(hwu_stack_heap_t* heap, hwu32 size, hwu32 alignment);
static void* allocate_from_bottom(hwu_stack_heap_t* heap, hwu32 size, hwu32 alignment);
...
void hwu_stack_heap_initialize(hwu_stack_heap_t* heap, void* memory, hwu32 size)
{
...
void* hwu_stack_heap_allocate_ex(hwu_stack_heap_t* heap, hwu32 size, hwu32 alignment, hwu_stack_heap_direction_t direction)
{
	HWU_NULL_ASSERT(heap);
...
{
	HWU_NULL_ASSERT(heap);
	direction == HWU_STACK_HEAP_DIRECTION_TOP ? deallocate_to_top(heap, maker) : deallocate_to_bottom(heap, maker);
}
